--[[
    ...
    Call with:
        ...
        FlyModule:Start()
        FlyModule:SetFlySpeed(speed <number>)
        FlyModule:Stop()

    Notes:
        [ ! ] This script does not support being called `require` multiple times on a single client.
        [ + ] This script supports mobile devices.
]]
...
local function setupControls()
	...
	local CharacterRemove = Services.Players.LocalPlayer.CharacterRemoving:Connect(function()
		FlyScript:Stop()
	end)

	table.insert(Objects.Connections, CharacterRemove)
end 
...
function FlyScript:SetFlySpeed(speed)
	Configuration.FlySpeed = speed
end
